...
class C_pembayaran_supplier_print extends MY_Controller
{
  ...
  public function index($id,$supplier_id)
  {
    ...
    foreach ($read_select as $key => $value) 
    {
      $no_pelanggan=$value->SUPPLIER;
      $no_form=$value->NO_FAKTUR;
    ...
    $total_hutang = 0;
    $vivo_paymen = 0;
    $read_select = $this->m_t_ak_pembayaran_supplier_rincian->select($id);
    foreach ($read_select as $key => $value) 
    {
      // BACA TERUTANG
      if($key == 0)
      { 
        $read_select_in = $this->m_t_ak_pembayaran_supplier->select_rincian_terutang($value->FAKTUR_PENJUALAN_ID);
        $sum_metode_bayar = 0;
        $sum_diskon = 0;
...
/*
    ...
    $read_select = $this->m_t_ak_pembayaran_supplier_print_setting->select_id(4);
    foreach ($read_select as $key => $value) 
    {
      $setting_value=$value->SETTING_VALUE;
    }
    ...
    $pdf->Output("pembayaran_supplier".$no_form.".pdf");
  }
