  ...
  public function index($pembelian_id)
  {
    ...
    for($i=0;$i<=$total_baris_transaksi;$i++)
    {
      ...
      if(($i>=$total_baris_1_bon and $rmd==0) or $i==0)
      {
        ...
        $pdf->Cell(30, 4, 'PT Cahaya Baru Gemilang', 0, 1, 'L');
      ...
      $pdf->MultiCell($colom_width[0], $baris_height, ($rmd).'.', '0', 'C',0,0);
      $pdf->MultiCell($colom_width[1], $baris_height, substr($kode_barang[$i], 0, 50), 'L', 'L',0,0);
      $pdf->MultiCell($colom_width[2], $baris_height, substr($barang[$i], 0, 50), 'L', 'L',0,0);
