        ...
        <tr>
          ...
          <th>
            <form action='/action_page.php'>
              <input type='date' class='form-control' name='date_po_manual' value='<?= $this->session->userdata('date_po_manual') ?>' onchange='this.form.submit();'>
          </th>
        </tr>
      </table>
  ...
  <div class="card-block">
    ...
    <div class="table-responsive dt-responsive">
      <table id="dom-jqry" class="table table-striped table-bordered nowrap">
        ...
        <tbody>
          <?php
          foreach ($c_t_t_t_po_manual as $key => $value) {
            if($value->MARK_FOR_DELETE == 'f')
            {
              ...
              echo "<td>";
              $ok_color = 'red';
              if ($value->T_STATUS != 10) {
                $ok_color = 'green';
              }
              if($value->INV_SUPPLIER!='' and $value->T_STATUS==10 and intval($value->SUM_SUB_TOTAL) != 0)
              {
                echo "<a href='" . site_url('c_t_t_t_po_manual/po_done/' . $value->ID) . "' ";
                ?>
              ...
              if ($value->SUM_SUB_TOTAL == 0) {
                ...
                echo "<a href='" . site_url('c_t_t_t_po_manual/delete/' . $value->ID) . "' ";
...
<form action="<?php echo base_url('c_t_t_t_po_manual/tambah') ?>" method="post" id='add_data'>
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Tanggal Transaksi:
            <form action='/action_page.php'>
              <input type='date' class='form-control' name='date' value='<?= $this->session->userdata('date_po_manual') ?>'>
          </h4>
...
<div class="modal fade" id="Modal_Edit" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <form action="<?php echo base_url('c_t_t_t_po_manual/edit_action') ?>" method="post" autocomplete="off" id='edit_data'>
      <div class="modal-content">
        <div class="modal-header">
...
<script>
  const read_data = <?= json_encode($c_t_t_t_po_manual) ?>;
