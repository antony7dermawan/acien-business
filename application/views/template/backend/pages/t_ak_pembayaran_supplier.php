        ...
        <tr>
          ...
          <th>
            <form action='/action_page.php'>
              <input type='date' class='form-control' name='date_pembayaran_supplier' value='<?= $this->session->userdata('date_pembayaran_supplier') ?>' onchange='this.form.submit();'>
          </th>
        </tr>
      </table>
      ...
      <table id="dom-jqry" class="table table-striped table-bordered nowrap">
        ...
        <tbody>
          ...
          foreach ($c_t_ak_pembayaran_supplier as $key => $value) {
            ...
            echo "<td>";
            echo "<a href='" . site_url('c_t_ak_pembayaran_supplier_rincian/index/' . $value->ID) . "/" . $value->SUPPLIER_ID . "' ";
            ...
            if(round($value->SUM_TOTAL_PENJUALAN)>0)
            {
              echo "<a href='" . site_url('c_t_ak_pembayaran_supplier_metode_bayar/index/' . $value->ID) . "/" . $value->SUPPLIER_ID . "' ";
              echo "onclick=\"return confirm('Isi Rincian?')\"";
              echo "> <i class='fa fa-search-plus text-c-blue'></i></a> ";
            }
            ...
            if(intval($value->SUM_TOTAL_PENJUALAN)!=0)
            {
              ...
              echo "{";
              echo "window.open('laporan_pdf/c_pembayaran_supplier_print/index/" . $value->ID . "/" . $value->PELANGGAN_ID . "');";
              echo "}";
              echo "</script>";
            ...
            if ($value->SUM_TOTAL_PENJUALAN == 0) {
              ...
              echo "<a href='" . site_url('c_t_ak_pembayaran_supplier/delete/' . $value->ID) . "' ";
...
<form action="<?php echo base_url('c_t_ak_pembayaran_supplier/tambah') ?>" method="post">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title"> Tanggal Transaksi
            <form action='/action_page.php'>
              <input type='date' class='form-control' name='date' value='<?= $this->session->userdata('date_pembayaran_supplier') ?>'>
          </h4>
