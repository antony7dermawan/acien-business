<div class="card">
  <div class="card-header">
      ...
      foreach ($c_t_t_t_pinlok_in_by_id as $key => $value) {
        $inv = $value->INV;
        $enable_edit = 1;
      }
  ...
  <div class="card-block">
    ...
    <a href="<?= base_url("c_t_t_t_pinlok_in"); ?>" class="btn waves-effect waves-light btn-inverse"><i class="icofont icofont-double-left"></i>Back</a>
    ...
    <div class="table-responsive dt-responsive">
      <table id="dom-jqry" class="table table-striped table-bordered nowrap">
        ...
        <tbody>
          <?php
          foreach ($c_t_t_t_pinlok_in_rincian as $key => $value) {
            ...
            if($value->MARK_FOR_DELETE == 'f')
            {
              ...
              {
                echo "<a href='".site_url('c_t_t_t_pinlok_in_rincian/delete/'.$value->ID.'/'.$pinlok_in_id)."' ";
...
<form action="<?php echo base_url('c_t_t_t_pinlok_in_rincian/tambah/'.$pinlok_in_id) ?>" method="post" id='add_data'>
...
function mycode() {
  ...
  tid = setTimein(mycode, 500); // repeat myself
}
