<div class="card">
  <div class="card-header">
    <form action='<?php echo base_url("c_t_t_t_retur_pembelian/date_retur_pembelian"); ?>' class='no_voucer_area' method="post" id=''>
      <table>
        <tr>
          <th>
            Tanggal Transaksi:
          ...
          <th>
            <form action='/action_page.php'>
              <input type='date' class='form-control' name='date_retur_pembelian' value='<?= $this->session->userdata('date_retur_pembelian') ?>' onchange='this.form.submit();'>
          </th>
        </tr>
      </table>
  ...
  <div class="card-block">
    ...
    <div class="table-responsive dt-responsive">
      <table id="dom-jqry" class="table table-striped table-bordered nowrap">
        ...
        <tbody>
          <?php
          foreach ($c_t_t_t_retur_pembelian as $key => $value) {
            if($value->MARK_FOR_DELETE == 'f')
            {
              ...
              //satu button
              echo "<td>";
              echo "<a href='" . site_url('c_t_t_t_retur_pembelian_rincian/index/' . $value->ID) . "' ";
              ...
              if (intval($value->SUM_SUB_TOTAL) != 0)
              {
                ...
                echo "{";
                echo "window.open('laporan_pdf/c_t_t_t_retur_pembelian_print/index/" . $value->ID . "');";
                echo "}";
                echo "</script>";
              }
            ...
            if($value->MARK_FOR_DELETE == 't')
            {
              ...
              echo "<a href='".site_url('c_t_t_t_retur_pembelian/undo_delete/' . $value->ID)."' ";
              ?>
...
<form action="<?php echo base_url('c_t_t_t_retur_pembelian/tambah') ?>" method="post" id='add_data'>
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Tanggal Transaksi:
            <form action='/action_page.php'>
              <input type='date' class='form-control' name='date' value='<?= $this->session->userdata('date_retur_pembelian') ?>'>
          </h4>
        ...
        <div class="form-group">
              ...
              <select name="pembelian_id" class='custom_width' id='select-state' placeholder='Pick a state...'>
              <?php
...
<div class="modal fade" id="Modal_Edit" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <form action="<?php echo base_url('c_t_t_t_retur_pembelian/edit_action') ?>" method="post" autocomplete="off" id='edit_data'>
      <div class="modal-content">
        <div class="modal-header">
...
<script>
  const read_data = <?= json_encode($c_t_t_t_retur_pembelian) ?>;
